...
const NavLink = ({ to, icon, children }: NavLinkProps) => {
    ...
    const isActive = location.pathname === `/dashboard${to === '/' ? '' : to}`;
    return (
        ...
        <aside className="md:w-64 flex-shrink-0">
            <div className="p-4 bg-white rounded-xl shadow-lg border border-gray-200 sticky top-24">
                ...
                <nav className="space-y-2">
                    <NavLink to="/" icon={<HomeIcon />} >Accueil Quiz</NavLink>
                    <NavLink to="/progression" icon={<ChartBarIcon />}>Ma Progression</NavLink>
                    <NavLink to="/resultats" icon={<DocumentTextIcon />}>Mes Notes</NavLink>
                    <NavLink to="/classement" icon={<TrophyIcon />}>Classement</NavLink>
                </nav>
            </div>
        </aside>
