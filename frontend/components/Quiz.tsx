...
export default function Quiz({ user }: QuizProps) {
  ...
  useEffect(() => {
    setLoading(true);
    if (user) {
      // Utilisateur connecté : charge tous les quiz depuis le serveur
      fetch(`${API_BASE_URL}/api/dashboard/quizzes`, { credentials: 'include' })
        ...
        .catch(err => {
            ...
            setQuizzes([]); // En cas d'erreur, la liste est vide
        })
        .finally(() => setLoading(false));
    } else {
        // Visiteur : ne voit que le quiz d'exemple
    ...
    <div className="space-y-12">
      ...
      {quizzes.length === 0 && !loading ? (
        <div className="text-center p-8 bg-white rounded-lg shadow-lg border">
            ...
            {user ? 
                <Link to="/dashboard" className="mt-4 inline-block bg-green-600 text-white px-6 py-2 rounded-lg hover:bg-green-700 font-semibold">Aller au Dashboard</Link> 
                : <p className="mt-2 text-gray-500">Inscrivez-vous pour accéder à plus de contenu !</p>
            }
        </div>
      ) : quizzes.map(quiz => (
          ...
          <form onSubmit={(e) => { e.preventDefault(); handleSubmit(quiz); }}>
            <div className="space-y-6">
              {quiz.questions.map((q, qIndex) => (
                // FIX: Wrap QuizQuestionCard in a React.Fragment to handle the 'key' prop, avoiding a TypeScript error.
                <React.Fragment key={q.key_name}>
                    <QuizQuestionCard 
                        q={q} 
                        qIndex={qIndex}
